...
const getRecentPost = async (req, res, next) => 
{
    ...
    //   const recentPost = await Activities.find().sort({ _id: -1 }).limit(3)
      ...
      let recentPost = await Posts.find({postedBy:username}).sort({ updatedAt: 'desc'}).exec().limit(10);
      if (!recentPost) {
        next(ApiError.notFound("No Post Exists"));
        return;
      }
      // const commentsCopy=getRecentComments;
      res.status(200).json({ message: "Sent the last 10 Activities.", recentPosts: recentPost});
    } catch (err) {
